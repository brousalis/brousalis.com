...
//	var c = ["#332f29","#8fa68e","#ccc59e","#cf3800"]
	var c = ["#405F6F","#92AA9C","#D1DCBC","#1A3341"]
	
	$('#content-header').css('background-color',c[3])
    $('#wrapper').append('<div class="get"><div class="arc"><span class="text">College</span><input type="hidden" class="percent" value="100" /><input type="hidden" class="color" value="'+c[0]+'" /></div><div class="arc"><span class="text">Age 22</span><input type="hidden" class="percent" value="'+days_left+'" /><input type="hidden" class="color" value="'+c[1]+'" /></div><div class="arc"><span class="text">27in Mac</span><input type="hidden" class="percent" value="72" /><input type="hidden" class="color" value="'+c[2]+'" /></div></div>');

	$('.posts article:not(:first-child)').addClass('list')

	$('#list-view').click(function () {
		$('.posts article').addClass('list');
		return false;
	});

	$('#grid-view').click(function () {
		$('.posts article').removeClass('list');
		return false;
	});
...
var o = {
	...
	diagram: function(){
		...
		var image = r.image("images/life2.png",105,125,52,12).toFront();
...
title=(''+title).replace(/(^\s*|\s*$)/,"");return title||o.fallback;},tip:function(){if(!this.$tip){pad=9+this.options.left
